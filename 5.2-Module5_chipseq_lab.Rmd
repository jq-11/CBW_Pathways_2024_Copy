# Module 5 Lab: Gene Regulation and Motif Analysis Practical Lab {#regulatory_network_chipseq_lab}
...
Perform pathway analysis starting with a chIP_seq bed file and visualized the results using Cytoscape/EnrichmentMap.Be able to use GREAT with distal and proximal parameters.Run MEME-chip to find over-enrichment of transcription factors.
Optional: learn how to use iRegulon to find targets of a transcription factor of interest and find orthologs using g:Profiler/g:orth

This practical consists of 6 exercises and 2 are them as optional. Follow the step-by-step checklist trough the exercises.  
...
*	[GSE128767_RUNX1_ChIP.peaks.bed](./Module5/chipseqlab/data/GSE128767_RUNX1_ChIP.peaks.bed)
*	[Distal_GOBP_greatExportAll.tsv](./Module5/chipseqlab/data/Distal_GOBP_greatExportAll.tsv)
*	[Proximal_GOBP_greatExportAll.tsv](./Module5/chipseqlab/data/Proximal_GOBP_greatExportAll.tsv)
*	[RUNX1_Affy.gmt](./Module5/chipseqlab/data/RUNX1_Affy.gmt)
*	[GSE128767_RUNX1_ChIP.peaks.fasta](./Module5/chipseqlab/data/GSE128767_RUNX1_ChIP.peaks.fasta)

```{block, type="rmd-note"}
Note about EnrichmentMap layout: Network layouts are flexible and can be rearranged. What you see when you perform these exercises may not be identical to what you see in the tutorial, or what you have seen other times that you have performed the exercises. 
```
...
*	Downloadable bed file from GEO 
*	Linked to transcriptomics data (GSE129038)
*	Quality of the data


Starting the analysis, the first thing that we need to check before doing the analysis is the model organism and genome version.In the GEO entry (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM3684638), the information is found by c clicking on one of the sample and under “Data processing” :
...
The first 3 fields contain the chromosome name, start and end. They are the 3 required fields. The fourth column is optional and consists here of the chromosomal position, followed by MACS2 score value and FDR. 
...
*Do not change the settings. We are doing a distal analysis where genes (genomic regions) are associated with peaks within 5k upstream of TSS, 1kb downstream and up to 1000 kb to nearest gene.
*Click on the “Submit” button at the end of the page

*Explore the results.
*After expanding the “Job Description” tab, in addition to the used parameters , you can click on “View all genomic region-gene associations” 

![img10](./Module5/chipseqlab/chipseqlab_image/img10.png)
![img10b](./Module5/chipseqlab/chipseqlab_image/img10b.png)
*In the “Region-Gene Association Graphs”, we can see that the peaks were mainly associated with genes located +-5kb of the TSS in addition to the presence of some distal peaks.

![distal](./Module5/chipseqlab/chipseqlab_image/distal.png)
*Let’s look at  the GO Biological Process table to explore the pathway analysis results.

![distal](./Module5/chipseqlab/chipseqlab_image/distal_true2.png)
As we defined a distal rule to associate peaks with genes, we are going to look at the binomial FDR. The binomial test assesses whether the number of the genomic regions annotated with the tested pathway is significantly larger than the number of the genomic regions not annotated with tested pathway.
...
*We are now trying a proximal approach to define genes associated with peaks. Go back to the GREAT page. 
...
*Explore the results.
*In the “Region-Gene Association Graphs” , we can see that according to the proximal rule in our settings, genes are associated with peaks that are all within the +-5kb rule (in fact the +-1kb rule).

![proximal](./Module5/chipseqlab/chipseqlab_image/proximal.png)

*Explore the GOBP results and export the results to your computer. 
![proximal2](./Module5/chipseqlab/chipseqlab_image/proximal2.png)

*Save the file as Proximal_GOBP_greatExportAll.tsv. 
![img13](./Module5/chipseqlab/chipseqlab_image/img13.png)
Using this rule, genes will be associated with peaks only if these ones are within 1kb of the transcription start site of the genes. It reduces the problem to a gene list and in this case, a Fisher’s exact test can be applied to test for pathway enrichment. 
...
## Exercise 3:Auto-annotate the distal proximal enrichment map and build the proximal map and compare both the proximal and distal maps
...
Arrange the display by clicking on each module name listed on the right tab and then move them apart from the other modules using a mouse a trackpad.
...
*Optional exercise 3b: Repeat the process of building an enrichment map using the Proximal data (Distal_GOBP_greatExportAll.tsv). Because this is proximal data, the problem is reduced to a gene list and you can use the Fisher’s exact test (FDR 0.001) to looked at the enrichment results
*Optional exercise 3c: Repeat the process by building both the Proximal and Distal enrichment maps at the same time. Check which nodes are in common between the 2. Use FDR 0.0001 for both and binomial test. Color the data by datasets. 

## Exercise 4: Add RUNX1 targets and RUNX1 KO up and down regulated genes on distal enrichment map as a new gene-set.

During this exercise, we will connect the pathway map with the RUNX1 targets and the genes that are dysregulated after RUNX1 KO. We have already created a .gmt file that contains these gene lists (RUNX1_Affy.gmt). The format of a .gmt file is a tab delimited text file with one row per gene-set . Each gene-set contains the name of the gene-set, a description of the gene-set followed by the names of the genes. The file extension is changed from .txt to .gmt.

![gmt](./Module5/chipseqlab/chipseqlab_image/gmt.png)

*Note: We found the RUNX1 targets using the iRegulon Cytoscape app and the optional exercise 6 describes the steps. We extracted 200 genes to built the RUNX1 target gene list.
This RUNX1 study had transcriptomics data (microarray) in addition to the chIP-seq data. The microarray data are giving an overview of all genes that are changing between a fetal ovary with normal development and a fetal ovary after RUNX1 KO (GSE129038). We have use the tool GEO2R to get the top 500 up and down regulated genes (see screenshot at the end the document). It is using a moderated t-test as statistics.

Add the .gmt file to the enrichment map:
* In Cytoscape,  go to the Network tab and make sure that the Distal_GOBP_greatExportAll network is selected.
* Go to the EnrichmentMap tab and click on “Add Signature Gene Sets…”; Click on “Load from File….” located on the right hand size and select the file “RUNX1_Affy.gmt” that you have saved on your computer.
...
Optional: In order to change the style:
...
*The first step is to format the data. MEME suite accepts sequences as input and not chromosome coordinates. The bed file contains the chromosome coordinated of the peaks. Therefore we first have to fetch all the peak sequences. UCSC genome browser (https://genome.ucsc.edu/) has some tools to help us.

*If needed, you can find the finalized formatted file here but I encourage you to follow the next steps to learn on how to do it yourself: GSE128767_RUNX1_ChIP.peaks.fasta
...
*Upload the bed file (GSE128767_RUNX1_ChIP.peaks.bed); make sure table browser is selected and press the go button. 
...
*You will need to modify the UCSC header that comes with the sequences to use them for meme. Go to https://fasta.bioch.virginia.edu/fasta_www2/clean_fasta.html and upload or copy and paste the plain text file from the above step; check Extract CHR:coordinates from UCSC ; Click on “Clean Sequence”.  Save this as plain text under the name GSE128767_RUNX1_ChIP.peaks.fasta
![meme7](./Module5/chipseqlab/chipseqlab_image/meme7.png)
![meme8](./Module5/chipseqlab/chipseqlab_image/meme8.png)

###Exercise 5b: Run MEME_chip

*Open https://meme-suite.org/meme/tools/meme-chip.
![meme9](./Module5/chipseqlab/chipseqlab_image/meme9.png)

*In Input the primary sequences box, upload the file GSE128767_RUNX1_ChIP.peaks.fasta . Click on “Start Search”. Important: Save the url so you can access your result later even if you close the MEME window. MEME-ChIP will run for about 1 hour: look at the results below from the MEME-chip result, try to answer the questions and follow next steps. Check your MEME-ChIP results at the end of the practical lab.
...
To which transcription factor does it correspond? Why is the centered distribution of the motif important (what does it mean)?
