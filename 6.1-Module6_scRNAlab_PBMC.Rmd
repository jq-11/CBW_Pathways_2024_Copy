# Module 6 lab 1: scRNA data downstream analysis {#scRNA-lab1}
...
## Introduction
As an example of applying pathway and network analysis using scRNA, we are taking the Seurat tutorial as starting point .This dataset is composed of Peripheral Blood Mononuclear Cells (PBMC) freely available from 10X Genomics. There are 2,700 single cells that were sequenced on the Illumina NextSeq 500 (https://satijalab.org/seurat/articles/pbmc3k_tutorial.html). 

```{block, type="rmd-note"}
The R code in this practical lab was used to produce the gene lists to use in the downstream analysis.
It is for your reference. YOU DON'T NEED TO RUN IT FOR THIS PRACTICAL LAB. ALL NECESSARY FILES ARE PROVIDED.
```
...
In the case of this dataset, we use canonical markers to easily match the clustering to known cell types:    
```
...
This part of the code find markers for every cluster compared to all remaining cells and report only the positive ones. These list of genes specific to each markers are the ones that we are going to use for further analysis.
```
...
### Gene list g:Profiler (Do not run code)
...
for (a in cluster_list){
  ...
  print(genelist)
  write.table(genelist, paste0(a, ".txt"), sep= "\t", col.names = F, row.names = F, quote=F)
}
```
...
For this practical lab, we will use the platelet gene list and use g:Profiler. 

 * Open the g:Profiler website at [g:Profiler](https://biit.cs.ut.ee/gprofiler/gost) in your web browser.
 * Open the file ([Platelet.txt](./scRNAlab/data/Platelet.txt)) in a simple text editor such as Notepad or Textedit to copy the list of genes.
 * Paste the gene list  into the Query field in top-left corner of the screen. 
...
<img src="./scRNAlab/images/scRNAlab4.png" alt="generate rank" width="75%" />
 
 * Save the results
  * In the *Detailed Results* panel, select "GEM" . 
  * keep the minimum term size set to 10 
  * set maximum term size to 500 
It will save the results in a text file in the "Generic Enrichment Map" format that we will use to visualize using Cytoscape.


<img src="./scRNAlab/images/scRNAlab5.png" alt="generate rank" width="75%" />


 * Rename the file to [gProfiler_platelet.txt](./scRNAlab/data/gProfiler_platelet.txt)
...
<img src="./scRNAlab/images/scRNAlab6a.png" alt="generate rank" width="75%" />

 * Drag and drop the g:Profiler file ([gProfiler_platelet.txt](./scRNAlab/data/gProfiler_platelet.txt))
 * Set **FDR q-value cutoff** to 0.001
 * Click on **Build**
 
<img src="./scRNAlab/images/scRNAlab6.png" alt="generate rank" width="75%" />

An enrichment map is created:

<img src="./scRNAlab/images/scRNAlab7.png" alt="generate rank" width="75%" />
...
<img src="./scRNAlab/images/scRNAlab8.png" alt="generate rank" width="100%" />

Note: The boxes **Palette** and **Word Wrap** have been selected. The clusters have been moved around for clarity.


### GSEA from pseudobulk
#### pseudobulk creation, differential expression and rank file

We also can create pseudobulk data from the scRNA data by summing all cells into defined groups. We used the clusters to group the cells and we are calculating  differential expression using edgeR. We are comparing the CD4 cells (Naive CD4 T and Memory CD4 T) and the monocytic cells (CD14+ Mono and "FCGR3A+ Mono) .
As show in previous module, to perform pathway analysis,we are preparing a rank file, run GSEA and create an enrichment map in Cytoscape.
...
* GSEA:
  * Open GSEA
   * Select **Load Data**
   * Drag and Drop the rank  [CD4vsMono.rnk](./scRNAlab/data/CD4vsMono.rnk) and gmt * [Human_GOBP_AllPathways_no_GO_iea_April_02_2023_symbol.gmt](./Module2/gsea/data/Human_GOBP_AllPathways_no_GO_iea_April_02_2023_symbol.gmt) files.
  ...
  * Click on **Run GSEAPreranked
   ...
   * Select the rank file: CD4vsMono
  * Expand **Basic Fields**
...
<img src="./scRNAlab/images/scRNAlab9.png" alt="generate rank" width="100%" />
...
<img src="./scRNAlab/images/scRNAlab10.png" alt="generate rank" width="100%" />
...
<img src="./scRNAlab/images/scRNAlab11.png" alt="generate rank" width="100%" />
...
colnames(raw) =   c("Naive_CD4_T", "CD14p_Mono", "Memory_CD4_T", "B", "CD8_T", "FCGR3Ap_Mono","NK", "DC", "Platelet")
saveRDS(raw, "raw.rds")
...
my.contrasts <- makeContrasts(CD4vsMono=myGroupsCD4-myGroupsMono, levels = design )
...
write.table(myrank, paste0(mycontrast, ".rnk"), sep="\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
