...
```{block, type="rmd-tip" }
In order to get all the results, even those that don’t pass correction, select *all results*.  By default, g:Profiler will only return the sets that pass the defined threshold. Often you need the ability to tweak the thresholds in the resulting EM beyond the strict FDR < 0.05 threshold and therefore require all the results. 
```
...
* By default, the "Overview" tab is selected. A global graph displays gene-sets that passed the significance threshold of 0.05 for the 3 gene-set databases that we have selected, GO Biological Process(GO:BP) and  Reactome(REAC) and WikiPathways(WP). Numbers in parentheses are indicating the number of gene-sets that passed the threshold.

<img src="./Module2/gprofiler/images/gp5.png" alt="workflow" width="100%" />

Step5b:

* Click on "Detailed Results" to view the results in more depth. Three tables are displayed, one for each of the data sources selected. (If more than 3 data sources are selected there will be additional tables for each datasource) Each row of the table contains:
...
7a.  In the *Detailed Results* panel, select "GEM" .  It will save the results in a text file in the "Generic Enrichment Map" format that we will use to visualize using Cytoscape.
   ...
   * set maximum term size to 10000 ( = no filtering by gene-set size) and click on the GEM button. A file is downloaded on your computer. (change the name to gProfiler_hsapiens_lab2_results_GEM_gs_min3_max10000.gem.txt)
  <p align="center">  
  <img src="./Module2/gprofiler/images/gp13.png" alt="workflow" width="75%" />
  </p>  
   * set maximum term size to 1000 ( = filter by gene-set size) and click on the GEM button. A file is downloaded on your computer. (change the name to gProfiler_hsapiens_lab2_results_GEM_gs_min3_max1000.gem.txt)
  <p align="center">
  <img src="./Module2/gprofiler/images/gp14.png" alt="workflow" width="75%" />
  </p>
   * select max term size to 250 ( = filter by gene-set size) and click on the GEM button. A file is downloaded on your computer. (change the name to gProfiler_hsapiens_lab2_results_GEM_gs_min3_max250.gem.txt)
  <p align="center">
...
Which term has the best corrected p-value?  <br> Which genes in our list are included in this term? <br> Observe that some genes can be present on several lines (pathways are related when they contain a lof of genes in common).
```
...
```{block, type="rmd-tip"}
The version info can be recorded anywhere (for example in your lab notebook) but a convenient place is to embedded it in the g:Profiler geneset file name used for the analysis.  <br>Instead of naming the file <br>
